...
export default function ProcurementPage() {
  ...
  const [sentOffers, setSentOffers] = useState<string[]>([]); // Tracks which offers we clicked
  ...
  return (
    <main className="p-4 md:p-8 relative z-10 w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
        ...
        {/* FARMER CARD 1 */}
        <div className="glass-panel p-6 rounded-[2rem] flex flex-col justify-between">
          <div>
          ...
          <button 
            onClick={() => handleSendOffer("Harpreet Singh", "Wheat (Lok-1)", "100 Quintals", "₹2,450", "card1")}
            disabled={sentOffers.includes("card1")}
            className={`w-full py-3 rounded-xl font-black transition-all flex justify-center items-center gap-2 ${
              sentOffers.includes("card1") 
          ...
          >
            {sentOffers.includes("card1") ? <><CheckCircle2 className="w-5 h-5"/> Offer Sent</> : "Send Offer"}
          </button>
        </div>
        ...
        <div className="glass-panel p-6 rounded-[2rem] flex flex-col justify-between">
          ...
          <button 
            onClick={() => handleSendOffer("Gurdeep Agro", "Potato (Kufri)", "50 Quintals", "₹1,150", "card2")}
            disabled={sentOffers.includes("card2")}
            className={`w-full py-3 rounded-xl font-black transition-all flex justify-center items-center gap-2 ${
              sentOffers.includes("card2") 
              ? "bg-[#10893E] text-white shadow-inner cursor-default" 
              : "bg-gradient-to-b from-[#FCD14D] to-[#FBC02D] text-[#0A2F1D] shadow-[0_6px_0_0_#D49800] hover:translate-y-[4px] hover:shadow-[0_2px_0_0_#D49800] border border-[#F5B921]"
            }`}
          >
            {sentOffers.includes("card2") ? <><CheckCircle2 className="w-5 h-5"/> Offer Sent</> : "Send Offer"}
          </button>
        </div>
